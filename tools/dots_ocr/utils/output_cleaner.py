...
class OutputCleaner:
    ...
    def clean_list_data(self, data: List[Dict], case_id: int) -> CleanedData:
        ...
        for i, item in enumerate(data):
            ...
            if "bbox" in item:
                ...
                if isinstance(bbox, list) and len(bbox) == 3:
                    print(
                        f"  ⚠️ Item {i}: bbox has only 3 coordinates. Removing bbox, keeping category and text."
                    )
        ...
        print(
            f"  ✅ Cleaning complete: {len(cleaned_data)} items, {operations['bbox_fixes']} bbox fixes, {operations['removed_items']} items removed"
        )
    ...
    def clean_string_data(self, data_str: str, case_id: int) -> CleanedData:
        ...
        try:
            ...
            data_str, duplicate_removes = (
                self._remove_duplicate_complete_dicts_preserve_order(data_str)
            )
    ...
    def _truncate_last_incomplete_element(self, text: str) -> Tuple[str, bool]:
        ...
        if needs_truncation:
            ...
            if bbox_count <= 1:
                print(
                    f"    ⚠️ Only {bbox_count} dict objects found, skipping truncation to avoid deleting all content"
                )
                return text, False
            ...
            if last_bbox_pos > 0:
                ...
                print(
                    f"    ✂️ Truncated the last incomplete element, length reduced from {len(text):,} to {len(truncated_text):,}"
                )
                return truncated_text, True

        return text, False

    def _remove_duplicate_complete_dicts_preserve_order(
        self, text: str
    ) -> Tuple[str, int]:
        ...
        if total_duplicates > 0:
            ...
            print(
                f"    ✅ Removed {total_duplicates} duplicate dicts, keeping {len(unique_dicts)} unique dicts (order preserved)"
            )
    ...
    def _handle_single_incomplete_dict(self, text: str) -> Optional[List[Dict]]:
        ...
        try:
            ...
            fixed_dict = {"bbox": bbox_coords, "category": category}
    ...
    def remove_duplicate_category_text_pairs_and_bbox(
        self, data_list: List[dict], case_id: int
    ) -> List[dict]:
        """Removes duplicate category-text pairs and duplicate bboxes"""

        if not data_list or len(data_list) <= 1:
            print(
                f"    📊 Data length {len(data_list)} <= 1, skipping deduplication check"
            )
            return data_list
        ...
        for pair_key, positions in category_text_pairs.items():
            if len(positions) >= 5:
                ...
                print(
                    f"    🔍 Found duplicate category-text pair: category='{category}', first 50 chars of text='{text[:50]}...'"
                )
                print(
                    f"        Count: {len(positions)}, removing at positions: {positions_to_remove}"
                )
        ...
        if not duplicates_to_remove:
            print(
                f"    ✅ No category-text pairs or bboxes found exceeding the duplication threshold"
            )
            return data_list
    ...
    def clean_model_output(self, model_output: str):
        try:
            ...
            if (
                ...
                and result.success
                and result.cleaned_data
            ):
                original_data = result.cleaned_data
                deduplicated_data = self.remove_duplicate_category_text_pairs_and_bbox(
                    original_data, case_id=0
                )
            ...
            if line.strip():
                try:
                    ...
                    if (
                        ...
                        print(
                            "🔄 Checking for and removing duplicate category-text pairs and bboxes..."
                        )
                        original_data = result.cleaned_data
                        deduplicated_data = (
                            self.remove_duplicate_category_text_pairs_and_bbox(
                                original_data, case_id
                            )
                        )
                        # Update the cleaned_data in the CleanedData object
                        result.cleaned_data = deduplicated_data
    ...
    def save_cleaned_data(self, output_dir: str):
        ...
        for result in self.cleaned_results:
            ...
            print(
                f"  ✅ Case {result.case_id}: {len(result.cleaned_data)} objects → {case_filename}"
            )
    ...
    def _generate_cleaning_report(self, output_dir: str):
        ...
        report.append(
            f"Processing Time: {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
        )
        report.append("")
        ...
        if list_results:
            ...
            for r in list_results:
                ...
                report.append(
                    f"  Case {r.case_id}: {ops['original_count']} → {ops['final_count']} objects"
                )
                if ops["bbox_fixes"] > 0:
                    report.append(f"    - bbox fixes: {ops['bbox_fixes']}")
                if ops["removed_items"] > 0:
                    report.append(
                        f"    - invalid items removed: {ops['removed_items']}"
                    )
            report.append("")
        ...
        if str_results:
            ...
            for r in str_results:
                ...
                report.append(
                    f"  Case {r.case_id} {status}: {ops['original_length']:,} chars → {ops['final_objects']} objects"
                )
                details = []
                ...
                if ops["duplicate_dicts_removed"] > 0:
                    details.append(
                        f"Duplicates removed: {ops['duplicate_dicts_removed']}"
                    )
                if details:
        ...
        report.append(
            "  ✅ The relative order of all dict objects is preserved during cleaning."
        )
        report.append(
            "  ✅ When deduplicating, the first occurrence of a dict is kept, and subsequent duplicates are removed."
        )
